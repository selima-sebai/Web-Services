...
async function load() {
  try {
    const budget = await apiGet("/budget");

    totalInput.value = budget.total || 0;

    const totalAllocated = Object.values(budget.allocations || {}).reduce((s, v) => s + (v || 0), 0);
    const totalActual = Object.values(budget.actuals || {}).reduce((s, v) => s + (v || 0), 0);
    const remainingUnallocated = (budget.total || 0) - totalAllocated;
    const remainingOverall = (budget.total || 0) - totalActual;

    summaryEl.innerHTML =
      `<div>Total: ${budget.total || 0}</div>` +
      `<div>Allocated: ${totalAllocated}</div>` +
      `<div>Actual spent: ${totalActual}</div>` +
      `<div>Remaining (unallocated): ${remainingUnallocated}</div>` +
      `<div>Remaining (overall): ${remainingOverall}</div>`;
      ...
      ? `<table style="width:100%"><tr><th>Category</th><th>Alloc</th><th>Actual</th><th>Remaining</th></tr>` +
        Object.keys(budget.allocations)
          .map((k) => {
            ...
            return `<tr><td>${k}</td><td>${alloc}</td><td>${actual}</td><td>${alloc - actual}</td></tr>`;
          })
          .join("") +
        `</table>`
    ...
    recordsEl.innerHTML = (budget.records || []).length
      ? `<ul>` +
        (budget.records || [])
          .map((r) => `<li>${r.date} — ${r.category}: ${r.amount} ${r.bookingId ? "(booking #" + r.bookingId + ")" : ""}</li>`)
          .join("") +
        `</ul>`
      : `<div>No records</div>`;
  } catch (err) {
...
addAllocBtn.addEventListener("click", async () => {
  ...
  try {
    await apiPut("/budget", { allocations: { [cat]: amt } });
    allocCategory.value = "";
    allocAmount.value = "";
    await load();
  } catch (err) {
